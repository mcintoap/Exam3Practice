    ...
    for i in range(len(sequence_of_sequences)):
        for k in range(len(sequence_of_sequences[i])):
            if type(sequence_of_sequences[i][k]) is int:
                list = list + [sequence_of_sequences[i][k]]
    return list
...
def run_test_big_letters():
    ...
    # Test 2:
    expected = 'WHATTHEHECK'
    answer = big_letters([(3, 1, 4),  # not a string
                          'WHAT',  # OTSSS
                          (10, 'Tjksdnaod', 10),  # not a string
                          [],  # not a string
                          'THesldkadpw',  # not a string
                          'Esdjasdoad',  #
                          'Hakdsaodmpw',  # not a string
                          '1 Ecansca !',  # OOPS
                          'C',  # A
                          'sdadwqad',  # PS
                          'K',  # B
    ...
    for k in range(len(sequence_of_sequences)):
        if type(sequence_of_sequences[k]) is str:
            for j in range(len(sequence_of_sequences[k])):
                if str(sequence_of_sequences[k][j]).isupper():
                    string = string + str(sequence_of_sequences[k][j])
    return string
