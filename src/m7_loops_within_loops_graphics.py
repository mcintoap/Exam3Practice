...
def hourglass(window, n, point, radius, color):
    ...
    for i in range(n):
        for j in range(i + 1):
            ...
            line2 = rg.Line(rg.Point(x2 - radius, y2),
...
def many_hourglasses(window, square, m, colors):
    ...
    for k in range(m):
        rect = rg.Rectangle(rg.Point(x - (k + 1) * radius, y - k * h - radius),
                            rg.Point(x + (k + 1) * radius, y + k * h + radius))
        rect.attach_to(window)
        window.render()
        hourglass(window, k + 1, rect.get_center(), radius, new_colors[k])
        x = x + radius * (2 * (k + 1) + 1)
